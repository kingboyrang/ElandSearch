...
@synthesize index;
@synthesize scroller;
@synthesize orginImage;
- (void)dealloc
{
    [_imageView release], _imageView = nil;
    if(_genieView){
       [_genieView release], _genieView = nil;
    }
    [super dealloc];
}
...
- (void)toggleChromeDisplay
{
    if (self.scroller&&[self.scroller respondsToSelector:@selector(toggleChromeDisplay)]) {
        SEL sel=NSSelectorFromString(@"toggleChromeDisplay");
        [self.scroller performSelector:sel withObject:nil];
    }
}
-(UIImage*)orginImage{
    if (_imageView.image) {
        return [_imageView.image copy];
    }
    return nil;
}
- (BOOL)isZoomed
...
{
    UIGraphicsBeginImageContextWithOptions(_imageView.bounds.size, YES, [[UIScreen mainScreen] scale]);
...
-(void)trashPhotoTargetFrame:(CGRect)frame{
    ...
    _isShow=YES;
    _genieView=[[AZGenieView alloc] initWithFrame:self.bounds];
    _genieView.backgroundColor=[UIColor clearColor];
    _genieView.delegate=self;
    _genieView.autoresizesSubviews=YES;
    _genieView.autoresizingMask=UIViewAutoresizingFlexibleHeight|UIViewAutoresizingFlexibleWidth;
    [self addSubview:_genieView];
    [self sendSubviewToBack:_genieView];
    _genieView.renderImage =[self screenshotForViewController];
    [_genieView setRenderFrame:_imageView.frame andTargetFrame:frame];
    _isShow = !_isShow;
    if (!_isShow)
        _imageView.hidden = YES;
    [_genieView genieAnimationShow:_isShow withDuration:1.0];
}
#pragma mark AZGenieAnimationDelegate
- (void)geineAnimationDone
{
    ...
    if (self.scroller&&[self.scroller respondsToSelector:@selector(finishTransh)]) {
        SEL sel=NSSelectorFromString(@"finishTransh");
        [self.scroller performSelector:sel withObject:nil];
    }
}
